 ...
 */
#ifndef POSITION_PRESSURE_MEASUREMENTMANAGER_H
#define POSITION_PRESSURE_MEASUREMENTMANAGER_H
...
#include <msf_updates/PositionPressureSensorConfig.h>

namespace msf_position_sensor {

typedef msf_updates::PositionPressureSensorConfig Config_T;
typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

class PositionPressureSensorManager : public msf_core::MSF_SensorManagerROS<
    msf_updates::EKFState> {
  typedef PositionSensorHandler<
      msf_updates::position_measurement::PositionMeasurement,
      PositionPressureSensorManager> PositionSensorHandler_T;
  friend class PositionSensorHandler<
      msf_updates::position_measurement::PositionMeasurement,
      PositionPressureSensorManager> ;
 public:
  ...
  PositionPressureSensorManager(
      ros::NodeHandle pnh = ros::NodeHandle("~/position_pressure")) {
    imu_handler_.reset(
        new msf_core::IMUHandler_ROS<msf_updates::EKFState>(*this, "msf_core",
                                                            "imu_handler"));

    position_handler_.reset(
        new PositionSensorHandler_T(*this, "", "position_pressure"));
    ...
    ReconfigureServer::CallbackType f = boost::bind(
        &PositionPressureSensorManager::Config, this, _1, _2);
    reconf_server_->setCallback(f);
  }
  virtual ~PositionPressureSensorManager() {
  }
 ...
 private:
  ...
  virtual void Config(Config_T &config, uint32_t level) {
    ...
    if ((level & msf_updates::PositionPressureSensor_INIT_FILTER)
        && config.core_init_filter == true) {
      Init(1.0);
      config.core_init_filter = false;
    }

    if ((level & msf_updates::PositionPressureSensor_SET_HEIGHT)
        && config.core_set_height == true) {
      Eigen::Matrix<double, 3, 1> p = position_handler_->GetPositionMeasurement();
  ...
  void Init(double scale) const {
    ...
    b_p
        << position_handler_->GetPositionMeasurement()(2) / scale
...
#endif  // POSITION_PRESSURE_MEASUREMENTMANAGER_H
